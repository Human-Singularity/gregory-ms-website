    ...
    return (
        <>
            ...
            {article.ml_predictions && article.ml_predictions.length > 0 && (
                <>
                    <p><strong className='text-muted'>Machine Learning Prediction (Highest Score)</strong>: <span id="ml_prediction_score">{Math.max(...article.ml_predictions.map(p => p.probability_score)).toFixed(3)}</span></p>
                </>
            )}
